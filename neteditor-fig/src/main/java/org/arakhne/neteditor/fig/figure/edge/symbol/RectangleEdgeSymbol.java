...
/** This class represents the "rectangle" symbols that will
 *  be paint in the ends of edges.
 *  <p>
 *  <center>
 *  	<img src="doc-files/RectangleEdgeSymbol.png"><br>
 *  	<i><u>Fig:</u>&nbsp;Example of a RectangleEdgeSymbol, which is not filled</i>
 *  </center>
 *  
 ...
 */
public class RectangleEdgeSymbol extends AbstractFillableEdgeSymbol {

	private static final long serialVersionUID = 3234441947851676253L;
	 ...
	 */
	public RectangleEdgeSymbol(float width, float height, boolean filled) {
		super(filled);
		this.width = width;
		this.height = height;
	}
	 ...
	 * @param height is the size of the rectangle along the edge vector.
	 */
	public RectangleEdgeSymbol(float width, float height) {
	 ...
	 * {@link #getLineColor()} is used.
	 */
	public RectangleEdgeSymbol(boolean filled) {
		this(5, 5, filled);
	}

	/** Reply the size of the rectangle perpendicularly to the
	 * ege vector.
	 *
	...
	/** Reply the size of the rectangle along the
	 * ege vector.
	 *
	...
	protected Shape2f getSymbolShape(float x, float y, float angle) {
		float alongSize = getHeight();
		float perpSize = getWidth();
		float dh = perpSize/2f;
		Path2f rectangle = new Path2f();
		rectangle.moveTo(x, y-dh);
		rectangle.lineTo(x+alongSize, y-dh);
		rectangle.lineTo(x+alongSize, y+dh);
