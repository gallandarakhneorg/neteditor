 ...
 *  <p>
 *  This reader supports the NetEditor/GXL specification version "1".
 *  <p>
...
public class GXLReader1 extends AbstractGXLReader {
	...
	private void extractFigures(Element node, Progression progression) throws IOException {
		...
		ProgressionUtil.init(progression, 0, children.getLength());
		...
		for(int i=0; i<children.getLength(); ++i) {
			...
			if (child instanceof Element) {
				...
				if (N_NODE.equals(child.getNodeName())) {
				...
				else if (N_REL.equals(child.getNodeName())) {

					String internalType = extractInternalType((Element)child);

					if (C_INTERNAL_COERCION_RELATION_TYPE.equals(internalType)) {
						String name = extractAttribute(String.class, (Element)child, "name", null); //$NON-NLS-1$
