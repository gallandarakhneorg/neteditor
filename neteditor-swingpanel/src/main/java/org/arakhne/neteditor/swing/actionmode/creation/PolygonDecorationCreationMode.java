...
public class PolygonDecorationCreationMode extends AbstractPolypointDecorationCreationMode {
	 ...
	 *                    contains this Mode.
	 */
	public PolygonDecorationCreationMode(boolean persistent, ActionModeManager<Figure,SwingViewGraphics2D,Color> modeManager) { 
	...
	protected DecorationFigure createFigure(Path2f path) {
		...
		while (iterator.hasNext()) {
			...
			case MOVE_TO:
			case LINE_TO:
				points.add(new Point2f(element.toX, element.toY));
				break;
			default:
	...
	protected void paintShape(SwingViewGraphics2D g,
		...
		if (candidate!=null) {
			...
			g.setStroke(
					VectorToolkit.stroke(1f, LineJoin.ROUND, EndCap.ROUND,
					1, new float[]{5, 5}, 0));
